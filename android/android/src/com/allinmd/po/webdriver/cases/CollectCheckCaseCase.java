...
public class CollectCheckCaseCase {
	...
	public void collectCase() {		
		TerminalWebElements.collect_case_btn.click();
		caseName = TerminalWebElements.case_name.getText();
		Assert.assertEquals("取消收藏", TerminalWebElements.collect_case_btn.getText(), "没有收藏成功，请检查后再试！");
	}
	...
	public void checkCollectCase() {
		...
		PerCenterWebElements.collect_content_case_btn.click();
		
		Assertion.assertEquals(caseName, PerCenterWebElements.collect_content_title.getText(), "收藏的病例列表第一个资源不符，请检查！");		
	}
	 ...
	 */
	@Test (priority = 4)
	public void cancelCollectCase() {
		PerCenterWebElements.collect_content_title.click();		
		Assertion.assertEquals("取消收藏", TerminalWebElements.collect_case_btn.getText(), "是未收藏状态，请检查！");	
		
		TerminalWebElements.collect_case_btn.click();
		Assertion.assertEquals("收藏", TerminalWebElements.collect_case_btn.getText(), "取消收藏失败，请检查！");	
	}
}
