...
public class DriverList {
	...
	/**
	 * 用AndroidDriver启动chrome浏览器
	 * @return
	 */
	public static AndroidDriver browserDriverRun() {
...
//        capabilities.setCapability("udid", Dom4jXml.getValue("udid"));								//设备ID 
		...
		try {
			Utils.restartAppium();
			browserdriver = new AndroidDriver(new URL("http://127.0.0.1:4723/wd/hub"), capabilities);
    ...
    public static DesiredCapabilities getCapabilities(int tag) {    
    	...
    	switch(tag){
			case 1:	     
				return setCapability("appPackage", "appActivity");
		        
			case 2:	
				return setCapability("rePackage", "reActivity");			
